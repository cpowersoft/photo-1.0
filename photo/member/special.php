...
elseif($job=="show_iframe"){
	...
	if($type=="list_atc"||$type=='all')
	{
		...
		while($rs = $db->fetch_array($query)){
			$rs[picurl]=tempdir($rs[picurl]);
			$rs[subject]="<a href='$Mdomain/bencandy.php?fid=$rs[fid]&id=$rs[aid]' target=_blank><img src='$rs[picurl]' width=120 height=90>$rs[title]</a>";
			$_listdb[$rs[aid]]=$rs;
		}
	...
	if($type=='myatc'||$type=='all')
	{
		...
		while($rs = $db->fetch_array($query)){
			...
			$show.="<tr align='center' class='trA' onmouseover=\"this.className='trB'\" onmouseout=\"this.className='trA'\"> 
                ...
                <td width='84%' align='left'><A HREF='$Mdomain/bencandy.php?fid=$rs[fid]&id=$rs[aid]' target=_blank><img src='$rs[picurl]' width=120 height=90>{$rs[title]}</A></td>
                <td width='11%'>&nbsp;$add</td>
              </tr>";
		}
		$show="<table width='100%' border='0' cellspacing='1' cellpadding='3'>
		...
		$sort_fid=str_replace("<select name='fid'","[<A target='spiframe'  HREF='special.php?job=show_iframe&id=$id&type=myatc&fid=$fid&ismy=1' style='color:$color_me;'>我的图片</A>] [<A target='spiframe'  HREF='special.php?job=show_iframe&id=$id&type=myatc&fid=$fid' style='color:$color_all;'>所有图片</A>]<select onChange='fid_jumpMenu(this)'",$sort_fid);
		echo "<SCRIPT LANGUAGE='JavaScript'>
		<!--
