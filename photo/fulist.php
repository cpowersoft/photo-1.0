...
$listdb=fuListThisSort($rows,$webdb[ListLeng]?$webdb[ListLeng]:50);		//本栏目图片列表
$page_sql=$webdb[viewNoPassArticle]?'':' AND yz=1 ';


$showpage=getpage("{$_pre}article A LEFT JOIN {$_pre}fu_article F ON A.aid=F.aid","WHERE F.fid=$fid $page_sql","fulist.php?fid=$fid",$rows);	//图片列表分页
