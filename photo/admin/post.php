...
if($job=='endHTML')
{
	...
	refreshto("$admin_path&file=artic&file=artic&job=listartic","<CENTER>[<A HREF='$admin_path&job=postnew&fid=$fid'>发表新主题</A>] [<A HREF='$admin_path&job=post_more&aid=$aid'>续发本主题</A>] [<A HREF='$admin_path&file=artic&job=listartic&fid=$fid'>返回图片列表</A>] [<A HREF='{$htmlurldb[showurl]}' target=_blank>查看图片</A>] [<A HREF='$admin_path&job=manage&aid=$aid'>修改图片</A>]</CENTER><div style='display:none;'><iframe src='$Murl/job.php?job=article_html&fid=$fid&aid=$aid' width=0 height=0></iframe></div>",60);
}
elseif($job=='manage')
{
	if($rsdb[pages]<2){
...
elseif($job=='post_more')
{
	if($step=='post')
	{
		...
		refreshto($FROMURL,"<CENTER>[<A HREF='$admin_path&job=postnew&fid=$fid'>发表新主题</A>] [<A HREF='$admin_path&job=post_more&aid=$aid'>续发本主题</A>] [<A HREF='$admin_path&file=artic&job=listartic&fid=$fid'>返回图片列表</A>] [<A HREF='$Murl/bencandy.php?fid=$fid&aid=$aid' target='_blank'>查看图片</A>] [<A HREF='$admin_path&job=manage&aid=$aid'>修改图片</A>]</CENTER>",60);
	}
	$MSG='续发图片';
...
elseif($job=='edit_more')
{
	if($step=='post')
	{
		...
		refreshto("$FROMURL","<CENTER>[<A HREF='$admin_path&job=postnew&fid=$fid'>发表新主题</A>] [<A HREF='$admin_path&job=post_more&aid=$aid'>续发本主题</A>] [<A HREF='$admin_path&file=artic&job=listartic&fid=$fid'>返回图片列表</A>] [<A HREF='$Murl/bencandy.php?fid=$fid&aid=$aid' target=_blank>查看图片</A>] [<A HREF='$admin_path&job=edit_more&aid=$aid&rid=$rid'>修改图片</A>]</CENTER>",60);
	}
	$MSG='修改图片';
	get_admin_html('post');
}
