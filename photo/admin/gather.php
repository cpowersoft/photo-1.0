...
elseif($action=="edit_page")
{
	...
	jump("设置成功","$admin_path&job=list",1);
}
...
elseif($action=="edit_content")
{
	...
	jump("修改成功,下一步你可以进行每篇内容页的多页采集设置","$admin_path&job=edit_page&id=$id",3);
}
...
elseif($action=="gather_content")
{
	...
	if($rsdb[show_morepage]&&!$morepage){
		...
		do{
			...
			}elseif($i<5&&file_get_contents($nexturl)){	//对于一些不把所有页都列出来的情况
				$moreurl_db[$page][]="$nexturl\t$title\t$i";
			}else{
				$i=0;
			}
	...
	if($urldb[$page]){
		...
		echo "来源:<A HREF='{$curl}' target='_blank'>{$curl}</A><hr>";
