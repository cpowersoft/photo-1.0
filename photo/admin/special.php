...
elseif($job=="show_iframe"){
	...
	//列出本专题下的图片,$type=='all',初始化时.$type=="list_atc",增加图片或移除图片时
	if($type=="list_atc"||$type=='all')
	{
		unset($_listdb,$show);
		$detail=explode(",",$rsdb[aids]);
		...
		while($rs = $db->fetch_array($query)){
			...
			$rs[subject]="<a href='$Murl/bencandy.php?fid=$rs[fid]&id=$rs[aid]' target='_blank'><img src='$rs[picurl]' width=90 height=60 border=0>$rs[title]</a>";
			$_listdb[$rs[aid]]=$rs;
		}
	...
	if($type=='myatc'||$type=='all')
	{
		...
		while($rs = $db->fetch_array($query)){
			...
			$show.="<tr align='center' class='trA' onmouseover=\"this.className='trB'\" onmouseout=\"this.className='trA'\"> 
                ...
                <td width='84%' style='border-bottom:1px dotted #ccc;' align='left'><A HREF='$Murl/bencandy.php?fid=$rs[fid]&id=$rs[aid]' target=_blank><img src='$rs[picurl]' width=90 height=60 border=0>{$rs[title]}</A></td>
                <td width='11%' style='border-bottom:1px dotted #ccc;'>&nbsp;$add</td>
              </tr>";
		}
		$show="<table width='100%' border='0' cellspacing='1' cellpadding='3'>
		...
		$sort_fid=str_replace("<select name='fid'","[<A target='spiframe'  HREF='$admin_path&job=show_iframe&id=$id&type=myatc&fid=$fid&ismy=1' style='color:$color_me;'>我的图片</A>] [<A target='spiframe'  HREF='$admin_path&job=show_iframe&id=$id&type=myatc&fid=$fid' style='color:$color_all;'>所有图片</A>]<select onChange='fid_jumpMenu(this)'",$sort_fid);
		echo "<SCRIPT LANGUAGE='JavaScript'>
		<!--
		parent.document.getElementById('show_myatc_fid').innerHTML=\"$sort_fid\";
		//-->
