...
//对用户发表图片与推荐图片的奖罚yz,unyz,com,uncom,del
...
function make_html_Function($fid,$id,$page=1,$P='',$linkcode=''){
	...
	if($id){
		...
		if($Html_Type[bencandy_id][$id]){				//某篇图片的个性URL
			$filename=$Html_Type[bencandy_id][$id];
		}elseif($Html_Type[bencandy][$fid]){			//某个栏目的图片个性URL
...
function make_html($content,$pagetype='N'){
	...
	if($pagetype=='bencandy')
	{
		if($Html_Type[bencandy_id][$id]){			//某篇图片的个性URL
			$filename=$Html_Type[bencandy_id][$id];
		}elseif($fidDB[bencandy_html]){				//某个栏目的图片个性URL
...
function delete_m_article($aid,$rid,$forcedel=0){
	...
	if($rsdb[topic]){
		if($forcedel||$webdb[ForceDel]){
			...
			$detail=explode("\n",$rsdb[photourl]);
			foreach($detail AS $key=>$value){
				list($url)=explode('@@@',$value);
				delete_attachment( $rsdb[uid],tempdir($url) );
			}
